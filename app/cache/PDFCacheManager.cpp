...
PreloadTask::PreloadTask(Poppler::Document* document, int pageNumber, CacheItemType type, QObject* target)
    : m_document(document), m_pageNumber(pageNumber), m_type(type), m_target(target)
{
    setAutoDelete(true);
}
    ...
    try {
        ...
        switch (m_type) {
            ...
            case CacheItemType::Thumbnail: {
                ...
                result = QPixmap::fromImage(image).scaled(128, 128, Qt::KeepAspectRatio, Qt::SmoothTransformation);
                break;
            }
            case CacheItemType::TextContent: {
        ...
        QMetaObject::invokeMethod(m_target, "onPreloadTaskCompleted", Qt::QueuedConnection);
        
    } catch (...) {
        qWarning() << "PreloadTask: Exception during preload of page" << m_pageNumber;
    }
}
    ...
    , m_settings(new QSettings("SAST", "Readium-Cache", this))
{
    ...
    connect(m_maintenanceTimer, &QTimer::timeout, this, &PDFCacheManager::performMaintenance);
    m_maintenanceTimer->start();
    
    // Load settings
    loadSettings();
    
    qDebug() << "PDFCacheManager initialized with max memory:" << m_maxMemoryUsage 
             << "bytes, max items:" << m_maxItems;
}

bool PDFCacheManager::insert(const QString& key, const QVariant& data, CacheItemType type, 
                            CachePriority priority, int pageNumber)
{
    ...
    qDebug() << "PDFCacheManager: Cached item" << key << "type:" << static_cast<int>(type)
             << "size:" << item.memorySize << "bytes";
...
bool PDFCacheManager::cacheRenderedPage(int pageNumber, const QPixmap& pixmap, double scaleFactor)
{
    QString key = generateKey(pageNumber, CacheItemType::RenderedPage, scaleFactor);
    return insert(key, pixmap, CacheItemType::RenderedPage, CachePriority::Normal, pageNumber);
}
...
bool PDFCacheManager::cacheThumbnail(int pageNumber, const QPixmap& thumbnail)
{
    ...
    return insert(key, thumbnail, CacheItemType::Thumbnail, CachePriority::High, pageNumber);
}
...
bool PDFCacheManager::cacheTextContent(int pageNumber, const QString& text)
{
    ...
    return insert(key, text, CacheItemType::TextContent, CachePriority::Normal, pageNumber);
}
...
void PDFCacheManager::enablePreloading(bool enabled)
{
    ...
    qDebug() << "PDFCacheManager: Preloading" << (enabled ? "enabled" : "disabled");
}

void PDFCacheManager::preloadPages(const QList<int>& pageNumbers, CacheItemType type)
{
    if (!m_preloadingEnabled) return;
...
QString PDFCacheManager::generateKey(int pageNumber, CacheItemType type, const QVariant& extra) const
{
    QString typeStr;
    switch (type) {
        case CacheItemType::RenderedPage: typeStr = "page"; break;
        case CacheItemType::Thumbnail: typeStr = "thumb"; break;
        case CacheItemType::TextContent: typeStr = "text"; break;
        case CacheItemType::PageImage: typeStr = "image"; break;
        case CacheItemType::SearchResults: typeStr = "search"; break;
        case CacheItemType::Annotations: typeStr = "annot"; break;
    }
...
bool PDFCacheManager::evictLeastUsedItems(int count)
{
    ...
    if (m_cache.isEmpty() || count <= 0) return false;
...
double PDFCacheManager::calculateEvictionScore(const CacheItem& item) const
{
    ...
    switch (item.priority) {
        case CachePriority::Low: score += m_lowPriorityWeight; break;
        case CachePriority::Normal: score += m_normalPriorityWeight; break;
        case CachePriority::High: score += m_highPriorityWeight; break;
        case CachePriority::Critical: score += 1000.0; break; // Should not be evicted
    }
    ...
    qint64 timeSinceLastAccess = QDateTime::currentMSecsSinceEpoch() - item.lastAccessed;
...
{
    m_maxMemoryUsage = m_settings->value("maxMemoryUsage", m_maxMemoryUsage).toLongLong();
    m_maxItems = m_settings->value("maxItems", m_maxItems).toInt();
    m_itemMaxAge = m_settings->value("itemMaxAge", m_itemMaxAge).toLongLong();
    m_evictionPolicy = m_settings->value("evictionPolicy", m_evictionPolicy).toString();
    m_preloadingEnabled = m_settings->value("preloadingEnabled", m_preloadingEnabled).toBool();
}
