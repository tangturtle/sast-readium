    ...
    , m_persistenceEnabled(false)
{
    ...
    m_cacheDirectory = QStandardPaths::writableLocation(QStandardPaths::CacheLocation) + "/readium";
    QDir().mkpath(m_cacheDirectory);
    
    qDebug() << "UnifiedCacheSystem: Initialized with" << m_config.maxMemoryUsage << "bytes limit";
}
...
void UnifiedCacheSystem::initializeTimers()
{
    ...
    connect(m_cleanupTimer, &QTimer::timeout, this, &UnifiedCacheSystem::onCleanupTimer);
    m_cleanupTimer->start();
    
    // Statistics timer
    m_statsTimer = new QTimer(this);
    m_statsTimer->setInterval(5000); // 5 seconds
    connect(m_statsTimer, &QTimer::timeout, this, &UnifiedCacheSystem::onStatsTimer);
    m_statsTimer->start();
}

bool UnifiedCacheSystem::insert(const QString& key, const QVariant& data, 
                               UnifiedCacheType type, UnifiedCachePriority priority, int pageNumber)
{
    ...
    while ((getCurrentMemoryUsage() + item.size) > m_config.maxMemoryUsage || 
        ...
        if (!evictLRUItems(1)) {
            qWarning() << "UnifiedCacheSystem: Failed to make space for new item";
            return false;
        }
    }
    ...
    qDebug() << "UnifiedCacheSystem: Cached" << key << "type:" << static_cast<int>(type) 
             << "size:" << item.size << "compressed:" << item.isCompressed;
...
bool UnifiedCacheSystem::cacheRenderedPage(int pageNumber, const QPixmap& pixmap, 
...
QPixmap UnifiedCacheSystem::getRenderedPage(int pageNumber, double scaleFactor, int rotation)
{
    QString key = generateKey(UnifiedCacheType::RenderedPage, pageNumber, 
                             QVariantList() << scaleFactor << rotation);
...
bool UnifiedCacheSystem::cacheThumbnail(int pageNumber, const QPixmap& thumbnail, const QSize& size)
{
...
void UnifiedCacheSystem::optimizeMemory()
{
    ...
    qDebug() << "UnifiedCacheSystem: Memory optimized, usage:" << getCurrentMemoryUsage();
}
...
void UnifiedCacheSystem::compressItem(UnifiedCacheItem& item)
{
    ...
    QByteArray compressed = compressData(originalData, m_config.defaultCompressionLevel);
    if (compressed.size() < originalData.size() * 0.9) { // Only if significant savings
...
QByteArray UnifiedCacheSystem::compressData(const QByteArray& data, CompressionLevel level) const
{
...
QByteArray UnifiedCacheSystem::decompressData(const QByteArray& compressedData) const
{
...
QString UnifiedCacheSystem::generateKey(UnifiedCacheType type, int pageNumber, const QVariant& params) const
{
    QString baseKey = QString("type_%1_page_%2").arg(static_cast<int>(type)).arg(pageNumber);
...
void UnifiedCacheSystem::updateStatistics(const UnifiedCacheItem& item, bool isHit)
{
...
void UnifiedCacheSystem::onCleanupTimer()
{
    performCleanup();
}

void UnifiedCacheSystem::onStatsTimer()
{
    updateStatistics();
}
...
void UnifiedCacheSystem::updateStatistics()
{
    ...
    if (m_stats.totalMemoryUsed > m_stats.maxMemoryLimit) {
        emit memoryThresholdExceeded(m_stats.totalMemoryUsed, m_stats.maxMemoryLimit);
    }
}
...
void UnifiedCacheSystem::loadSettings()
{
    ...
    m_config.maxMemoryUsage = m_settings->value("cache/maxMemoryUsage", 512 * 1024 * 1024).toLongLong();
    m_config.maxItems = m_settings->value("cache/maxItems", 1000).toInt();
    m_config.enableCompression = m_settings->value("cache/enableCompression", true).toBool();
    m_config.defaultCompressionLevel = static_cast<CompressionLevel>(
        m_settings->value("cache/compressionLevel", static_cast<int>(CompressionLevel::Fast)).toInt());
    m_config.cleanupInterval = m_settings->value("cache/cleanupInterval", 30000).toInt();
}
...
void UnifiedCacheSystem::saveSettings()
{
    ...
    m_settings->setValue("cache/compressionLevel", static_cast<int>(m_config.defaultCompressionLevel));
