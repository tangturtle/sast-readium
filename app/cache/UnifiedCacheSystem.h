...
struct UnifiedCacheItem {
    ...
    UnifiedCacheItem()
        ...
        , size(0), timestamp(0), lastAccessed(0)
        , accessCount(0), pageNumber(-1)
        , isCompressed(false)
...
struct UnifiedCacheStats {
    ...
    UnifiedCacheStats()
        : totalItems(0), totalMemoryUsed(0), maxMemoryLimit(0)
        , hitCount(0), missCount(0), hitRate(0.0)
        , compressionSaved(0), compressedItems(0) {}
};
...
public:
    ...
    bool insert(const QString& key, const QVariant& data, 
               UnifiedCacheType type, UnifiedCachePriority priority = UnifiedCachePriority::Normal,
               int pageNumber = -1);
    ...
    bool cacheRenderedPage(int pageNumber, const QPixmap& pixmap, double scaleFactor = 1.0, int rotation = 0);
    QPixmap getRenderedPage(int pageNumber, double scaleFactor = 1.0, int rotation = 0);
    
    bool cacheThumbnail(int pageNumber, const QPixmap& thumbnail, const QSize& size = QSize());
    ...
    // Compression
    QByteArray compressData(const QByteArray& data, CompressionLevel level) const;
...
private:
    ...
    QString generateKey(UnifiedCacheType type, int pageNumber, const QVariant& params = QVariant()) const;
    ...
    qint64 calculateCompressionSavings(const QByteArray& original, const QByteArray& compressed) const;
};
