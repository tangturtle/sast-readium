    ...
    , m_maxRecentFiles(DEFAULT_MAX_RECENT_FILES)
{
    ...
    qDebug() << "RecentFilesManager: Initialized with max files:" << m_maxRecentFiles;
}
...
void RecentFilesManager::setMaxRecentFiles(int maxFiles)
{
    if (maxFiles < 1 || maxFiles > 50) {
        qWarning() << "RecentFilesManager: Invalid max files count:" << maxFiles;
        return;
    }
...
void RecentFilesManager::cleanupInvalidFiles()
{
    ...
    while (it != m_recentFiles.end()) {
        if (!it->isValid()) {
            qDebug() << "RecentFilesManager: Removing invalid file:" << it->filePath;
...
void RecentFilesManager::initializeAsync()
{
    ...
    QTimer::singleShot(100, this, [this]() {
        try {
            ...
            // 检查对象是否仍然有效
            if (!m_settings) {
                qWarning() << "RecentFilesManager: Settings object is null during async cleanup";
                return;
            }

            cleanupInvalidFiles();
            qDebug() << "RecentFilesManager: Async cleanup completed successfully";

        } catch (const std::exception& e) {
            qWarning() << "RecentFilesManager: Exception during async cleanup:" << e.what();
        } catch (...) {
            qWarning() << "RecentFilesManager: Unknown exception during async cleanup";
        }
    });
}
...
void RecentFilesManager::loadSettings()
{
    ...
    qDebug() << "RecentFilesManager: Loaded and cleaned" << m_recentFiles.size() << "recent files";
}
...
void RecentFilesManager::loadSettingsWithoutCleanup()
{
    ...
    // 加载最大文件数量
    m_maxRecentFiles = m_settings->value(SETTINGS_MAX_FILES_KEY, DEFAULT_MAX_RECENT_FILES).toInt();
    ...
    qDebug() << "RecentFilesManager: Loaded" << m_recentFiles.size() << "recent files (without cleanup)";
}
...
QVariantMap RecentFilesManager::fileInfoToVariant(const RecentFileInfo& info) const
{
    QVariantMap data;
...
RecentFileInfo RecentFilesManager::variantToFileInfo(const QVariantMap& variant) const
{
    RecentFileInfo info;
