...
struct MemoryBlock {
    ...
    MemoryBlock()
        : ptr(nullptr), size(0), actualSize(0), inUse(false)
        , timestamp(0), poolType(MemoryPoolType::SmallObjects), refCount(0) {}
};
...
struct MemoryPoolStats {
    ...
    MemoryPoolStats()
        ...
        , blockCount(0), freeBlocks(0), fragmentation(0.0)
...
public:
    ...
    static QByteArray compress(const QByteArray& data, CompressionType type = LZ4);
    static QByteArray decompress(const QByteArray& compressedData, CompressionType type = LZ4);
    static double getCompressionRatio(const QByteArray& original, const QByteArray& compressed);
...
public:
    ...
    // Memory allocation
    void* allocate(size_t size, MemoryPoolType poolType = MemoryPoolType::MediumObjects);
    void deallocate(void* ptr);
