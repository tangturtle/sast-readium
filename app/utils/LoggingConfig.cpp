...
const QList<LoggingConfig::SinkConfiguration>
    LoggingConfig::s_defaultSinkConfigs = {
        {"console", "console", Logger::LogLevel::Debug,
         "[%Y-%m-%d %H:%M:%S.%e] [%^%l%$] %v", true},
        {"file", "rotating_file", Logger::LogLevel::Info,
         "[%Y-%m-%d %H:%M:%S.%e] [%n] [%l] %v", true}};
        ...
        if (json.contains("categories")) {
            ...
            for (const auto& value : categoriesArray) {
                CategoryConfiguration config =
                    categoryConfigFromJson(value.toObject());
                m_categoryConfigs.append(config);
            }
        }
...
Logger::LogLevel LoggingConfig::parseLogLevelFromString(
...
void LoggingConfig::globalConfigToSettings(
...
void LoggingConfig::sinkConfigToSettings(
...
void LoggingConfig::categoryConfigToSettings(
...
LoggingConfig::GlobalConfiguration LoggingConfig::globalConfigFromSettings(
    QSettings& settings) const {
    GlobalConfiguration config;
    config.globalLevel =
        stringToLogLevel(settings.value("globalLevel", "info").toString());
...
LoggingConfig::SinkConfiguration LoggingConfig::sinkConfigFromSettings(
    QSettings& settings) const {
    SinkConfiguration config;
...
LoggingConfig::CategoryConfiguration LoggingConfig::categoryConfigFromSettings(
    QSettings& settings) const {
    CategoryConfiguration config;
    config.name = settings.value("name").toString();
    config.level = stringToLogLevel(settings.value("level", "info").toString());
...
LoggingConfig::GlobalConfiguration LoggingConfig::globalConfigFromJson(
...
LoggingConfig::SinkConfiguration LoggingConfig::sinkConfigFromJson(
    const QJsonObject& json) const {
    SinkConfiguration config;
...
LoggingConfig::CategoryConfiguration LoggingConfig::categoryConfigFromJson(
    const QJsonObject& json) const {
    CategoryConfiguration config;
    config.name = json["name"].toString();
    config.level = stringToLogLevel(json["level"].toString("info"));
