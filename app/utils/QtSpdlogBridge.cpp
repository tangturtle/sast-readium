...
void QtSpdlogBridge::qtMessageHandler(QtMsgType type,
                                      const QMessageLogContext& context,
                                      const QString& message) {
    QtSpdlogBridge::instance().handleQtMessage(type, context, message);
}

void QtSpdlogBridge::handleQtMessage(QtMsgType type,
                                     const QMessageLogContext& context,
                                     const QString& message) {
...
QString QtSpdlogBridge::formatQtMessage(QtMsgType type,
                                        const QMessageLogContext& context,
...
SpdlogQDebug& SpdlogQDebug::operator<<(const QRect& rect) {
    m_stream << "QRect(" << rect.x() << "," << rect.y() << " " << rect.width()
             << "x" << rect.height() << ")";
    return *this;
}
...
SpdlogQDebug spdlogDebug() { return SpdlogQDebug(Logger::LogLevel::Debug); }

SpdlogQDebug spdlogInfo() { return SpdlogQDebug(Logger::LogLevel::Info); }

SpdlogQDebug spdlogWarning() { return SpdlogQDebug(Logger::LogLevel::Warning); }
...
SpdlogLoggingCategory::SpdlogLoggingCategory(const char* category)
    : m_categoryName(QString::fromLatin1(category)),
      m_enabledLevel(Logger::LogLevel::Debug) {}
